function  collapse_time_row = get_collapse_time_row_variablestrainnonlinear(row_H, row_dhdt, row_epsxx, row_mdot, row_tags, row_dt, row_tmax,...
...
for iy = 1:ny
    if ((row_tags(iy) == 6) || (row_tags(iy)==4) || (row_tags(iy) == 3))
        ...
        if row_tags(iy) == 6 %have all data
            collapse_time_row(iy) = get_collapse_time_advect_variablestrainnonlinear(pp, dt, tmax);
