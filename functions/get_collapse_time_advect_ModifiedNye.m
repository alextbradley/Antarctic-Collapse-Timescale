function collapse_time = get_collapse_time_advect_ModifiedNye(pp, dt, tmax)
...
dimless_crev_depth = get_dimless_crev_depth_ModifiedNye(pp, anonT); %time zero crevasse depth

while (i < nsteps) &&  (dimless_crev_depth < 1)
    anonT = @(z) TgfF(z) + (pp.Tb- TgfF(z)).*exp(-z/pp.l); %with advected grounding line contribution
    %anonT = @(z) min(anonT(z), 273.15);  %don't let it go above freezing
    dimless_crev_depth = get_dimless_crev_depth_ModifiedNye(pp, anonT);


    %update thickness and time
   % [dimless_crev_depth, pp.H0]
