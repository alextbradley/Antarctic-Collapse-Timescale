...
cmap = flipud(cmocean('balance', 100));
...
%project onto the l axis
ls = length(shelf_names);
scatter3(ax(2),min(ax(2).XLim)*ones(1,ls), epsxx_ave,1./thinrate_ave,ms, shelf_cols, 'Filled');

% project onto the epsxx axis
scatter3(ax(2), ell_ave, max(ax(2).YLim)*ones(1,ls),1./thinrate_ave,ms, shelf_cols, 'Filled');

%project onto the thinning rate axis
scatter3(ax(2), ell_ave,  epsxx_ave ,min(ax(2).ZLim)*ones(1,ls),ms,shelf_cols, 'Filled');
